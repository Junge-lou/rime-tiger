...
local function test_shared_user_word_management_order()
  local base = read("tiger_base.schema.yaml")
  assert_processor_order(base, "lua_processor@*tiger_user_words_manager*processor", "lua_processor@*tiger_user_words*processor", "manager processor runs before user words")
  ...
  assert_order(base, "lua_translator@*tiger_user_words_manager*translator", "lua_translator@calculator_translator", "manager translator runs before calculator")
  assert_contains(base, "user_words_manager: \"^\\\\gl[hou]?[a-z;']*$\"", "base recognizes manager routes")
  ...
  assert_processor_order(example, "lua_processor@*tiger_user_words_manager*processor", "lua_processor@*tiger_user_words*processor", "schema example manager runs first")
  ...
  assert_contains(example, "lua_translator@*tiger_user_words_manager*translator", "schema example includes manager translator")
  assert_contains(example, "user_words_manager: \"^\\\\gl[hou]?[a-z;']*$\"", "schema example recognizes manager routes")
...
local function test_user_word_management_documentation()
  for _, path in ipairs({ "README.md", "配置说明/配置说明.txt" }) do
    ...
    assert_contains(text, "Shift+Delete", path .. " documents shift delete")
    assert_contains(text, "Ctrl+Delete", path .. " documents control delete")
    assert_contains(text, "Ctrl+Shift+M", path .. " documents candidate management")
    assert_not_contains(text, "Ctrl+;", path .. " removes the old add shortcut")
    assert_not_contains(text, "Ctrl+'", path .. " removes the old delete shortcut")
