...
runtime_case("capture metadata and live mode status", function()
  ...
  assert(output[1].comment:find("[中文取词]", 1, true))
  assert(output[1].comment:find("Enter", 1, true))
  assert(output[1].comment:find("Esc", 1, true))
  assert(output[1].comment:find("Backspace", 1, true))
  env.engine.context:set_option("ascii_mode", true)
  output = filter_output(env)
  assert(output[1].comment:find("[英文直录]", 1, true))
