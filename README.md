...
输入 `\dcck`，选择“确认导出当前方案”。程序会自动识别当前方案：`tiger` 导出为 `tiger_export.txt`，`tigress` 导出为 `tigress_export.txt`；仅支持这两个方案。文件保存在 Rime 用户数据目录中，使用 UTF-8（无 BOM）和 LF 换行，每行格式为 `词<Tab>编码`，不含权重或表头。

导出内容包括静态字词、手工维护的用户词典条目、标点与符号，以及通过快捷操作产生的加词、禁用和调序结果。含有字面 Tab 或其他控制字符的不安全条目会跳过。导出仅在选择命令后同步执行，平时输入不会承担解析开销。
...
- `编码 + \\ + Space`：在目标编码后连续输入两个反斜杠，再按空格进入加词模式。
- `Shift+Delete` 或 `Ctrl+Delete`：删除当前高亮候选。
- `Ctrl+Shift+M`：有编码时管理当前编码；空输入时打开全局管理。
- `Enter`：保存加词；`Esc`：取消；`Backspace`：优先删除候选查询编码，查询为空时删除已取内容的最后一个 Unicode 字符。
...
加词时通常输入小写编码并选择中文候选。例如，加“你好”时，依次输入“你”“好”的小写编码并各选一次候选。

需要英文时，轻按平时用于中英文切换的 `Shift`，即可切到英文直输。例如，加“周末plan”时，先选出“周”“末”，再直输小写 `plan`。英文直输还支持大写字母、数字、空格、符号和小键盘内容；再轻按一次 `Shift`，恢复中文候选查找。`Shift+字母` 会直接记录大写字母，不切换语言状态。中文查询为空时，也可直接加入大写字母、数字或符号，组成 `A4纸` 这类混合词条。保存或取消后，会恢复进入加词时的中英文状态。

官方词也可以删除。删除采用 LevelDB 软删除，不修改官方 YAML；更新码表后仍保持隐藏，并可通过 `Ctrl+Shift+M` 恢复。若同码同词既是快捷加词又是官方词，删除会取消快捷新增并隐藏官方候选，保证它不再出现。

当前编码管理会重新显示隐藏项，可用删除键恢复；`Ctrl+0` 清除当前编码的隐藏和手动调序。空输入进入的全局管理分为“已隐藏”“手动调序”“快捷加词”，可继续输入编码筛选。全局管理只操作快捷记录，不编辑 `tiger.user.dict.yaml` 或 `tigress.user.dict.yaml`。

`tiger` 和 `tigress` 的加词、删除、调序状态写入各自独立的 Rime 用户数据库，不会修改当前方案的官方码表。首次迁移后，`tiger.user.dict.yaml` / `tigress.user.dict.yaml` 仍可作为手工用户词典使用；通过快捷键产生的记录不再依赖这两个文件。
